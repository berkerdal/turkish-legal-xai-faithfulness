"""Nihai istatistik: 5 yöntem (4 + rastgele). Friedman + çiftli Wilcoxon.
Özellikle her gerçek yöntem 'random'dan anlamlı iyi mi? (anti-faithfulness kontrolü)"""
...
out = {}
for metric, better in [("comprehensiveness", "yuksek iyi"), ("sufficiency", "dusuk iyi")]:
    piv = raw.pivot(index="idx", columns="method", values=metric)[methods].dropna()
    n = len(piv)
    fr = friedmanchisquare(*[piv[m] for m in methods])
    print(f"\n===== {metric} ({better}) | n={n} =====")
    print("ortalama:", {m: round(float(piv[m].mean()), 4) for m in methods})
    ...
    print(f"{'cift':42s} {'Δort':>8s} {'p':>9s}  sig")
...
print("\nKaydedildi: results/faithfulness_stats_final.json")
